...
export class ServicesPage implements OnInit, OnDestroy {
          ...
          this.photos = _.filter(
            ...
            (document: FileI) =>
              ...
              ((document.lang !== 'null' && document.lang === this.user.lang) ||
                ...
                document.lang === 'null') &&
              document.category ===
                id /*&& document.serviceCategory === this.currentPage*/
          );
          this.getLocalUrl();
        }
      });
  ...
  private loadFiles(category: string): void {
    ...
    const documents = _.filter(
      this.allDocuments,
      (document: FileI) => document.category === category
    );
  ...
  private loadServicesCards(): void {
    this.servicesCategories = [];
    const services: any = _.orderBy(
      _.filter(SERVICES_CATEGORIES, (service: any) => service.home),
      ['label']
    );
